var bookappointment={
		...
		URL:{
			...
			appoint:function(bookId,studentId){
				...
				return projectName+ '/books/' + bookId + '/appoint?studentId='+studentId;
			},
			verify:function(){
				var pathName=window.document.location.pathname;
				 var projectName=pathName.substring(0,pathName.substr(1).indexOf('/')+1);
				return  projectName+ '/books' + '/verify';
			}
		},
		...
		validateStudent:function(studentId,password){
			...
			if(!studentId||!password){
				return "nothing";
			}else if(isNaN(studentId)||isNaN(password)){
				return "typerror";
			}else {
				...
				if(!studentId||!password){
					...
					$('#studentBtn').click(function (){
						...
						if(temp=="nothing"){
							...
							$.cookie('studentId', studentId, {  path: projectName+'/books'}); 
							$.cookie('password', password, {  path: projectName+'/books'}); 
							// 跳转到预约逻辑 
